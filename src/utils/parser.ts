...
import javaLanguageUrl from '/tree-sitter-java.wasm?url'
...
export namespace java {
  ...
  export async function init() {
    ...
    const Java = await Parser.Language.load(javaLanguageUrl)
    parser.value.setLanguage(Java)
  }
  ...
  export function parseTree(content: string): Tree | undefined {
    return parser.value?.parse(content)
  }
  export function parse(path: string, content: string): JavaFileMeta {
    ...
    const rootNode = parser.value?.parse(content).rootNode
