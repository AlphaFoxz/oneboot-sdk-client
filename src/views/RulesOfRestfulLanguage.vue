...
<template>
  <div style="min-height: 100%">
    ...
    <div class="bg-white text-black">
      ...
      <p>
        <b>enum</b
        >：当作为字段类型时，表示对枚举的引用（文档级别），但实际上所有enum类型的字段在代码中都是i32类型。但是因为以这种方式进行了引用，所以在代码会生成对应的枚举供开发使用，后续的文档生成也会更详细。<br />用法：<br /><code
          >/*声明一个性别字段*/<br />enum&lt;GenderEnum> gender</code
        >
      </p>
      <p>
        <b>void</b>：无类型，用在接口返回值，表示不关心返回值，只要请求成功就行了<br />用法：<br /><code
          >/*埋点，推送用户操作数据*/<br />void userHistory(list&lt;string> data)</code
        ><br /><b>注：通常埋点会选择udp而非tcp协议，所以此功能暂定</b>
      </p>
      <h2>修饰语</h2>
      <p>
        <b>required</b
